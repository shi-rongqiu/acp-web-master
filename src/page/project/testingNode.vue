        ...
        <div v-for="(item, index) in data" :key="item.id">
          <div v-if="item.processStatus == 0" class="content wait">
            ...
            <div class="foot-content">
              ...
              <span class="startBtn" :class="{'foot':hasTest == 1}" @click="startDetection(item, index)"><img src="../../assets/image/start-icon.svg" alt="">开始检测</span>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    startDetection (item, index) {
      if (this.hasTest == 0) {
        mylib.axios({
          url: '/project/checkPreCaseIfSucc',
          params: {
            projectCode: this.projectCode,
            useCasesCode: item.useCasesCode
          }
        }).then((res) => {
          if (res.code == 200) {
            mylib.axios({
    ...
    WebSocketSet () {
      if ('WebSocket' in window) {
        ...
        this.websock.onmessage = (res => {
          ...
          console.log(rst)
          // type 1结束 2开始 3过程
          if ((rst.type == 1 || rst.type == 4) && rst.data.projectCode == this.projectCode) {
            this.getParam()
            this.websock.close()
            this.hasTest = 0
            sessionStorage.setItem('hasTest', 0)
