...
<!--      <chart id="chart1" :options="myChart" @click="clickFun" class="mychart"></chart>-->
      <TreeChart :json="data" @click-node="clickNode" style="margin:50px auto;"/>
    </div>
<!--    右侧操作区-->
...
<!--                  style="color: #af5a00;font-size: 14px;">-->
<!--                  删除-->
<!--                </el-button>-->
<!--                <el-button-->
<!--                  type="text"-->
  ...
  methods: {
    ...
    changeTree (data) {
      ...
      data.children.forEach((al) => {
        ...
        // }
        var res = ''
        if (al.chs && al.chs.length > 0) {
          al.chs.forEach((el) => {
            if (el.chStatus == 1) {
              res = el.ch + '#' + el.freq + 'M#开启' + ',' + res
            } else if (el.chStatus == 0) {
              res = el.ch + '#关闭' + ',' + res
            }
          })
          ...
          al.children.forEach((bl) => {
            ...
            if (bl.children && bl.children.length > 0) {
              bl.children.forEach((el) => {
                ...
                el.image_url = 'https://ss0.bdstatic.com/70cFvHSh_Q1YnxGkpoWK1HF6hhy/it/u=3689173839,956040439&fm=26&gp=0.jpg'
                // el.lineStyle = {
                //   color: '#ff8400'
                // }
                // el.itemStyle = {
                ...
                if (el.children && el.children.length > 0) {
                  el.children.forEach((cl) => {
                    ...
                    cl.image_url = 'https://ss0.bdstatic.com/70cFvHSh_Q1YnxGkpoWK1HF6hhy/it/u=3689173839,956040439&fm=26&gp=0.jpg'
                    // cl.lineStyle = {
                    //   color: '#f84848'
                    // }
                    // cl.itemStyle = {
